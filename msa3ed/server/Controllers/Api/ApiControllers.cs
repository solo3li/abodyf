...
public class ServicesController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<IActionResult> GetAll(string? search, Guid? categoryId, decimal? minPrice, decimal? maxPrice, string? sortBy = "newest")
    {
...
public class PaymentsController : ControllerBase
{
    ...
    [HttpPost("{orderId}")]
    public async Task<IActionResult> Process(Guid orderId, [FromBody] decimal amount)
    {
...
public class ChatController : ControllerBase
{
    ...
    [HttpGet("Inbox")]
    public async Task<IActionResult> GetInbox([FromQuery] string filter = "All")
    {
    ...
    public async Task<IActionResult> GetOrderChat(Guid orderId)
    {
        var myIdStr = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (myIdStr == null) return Unauthorized();
        var myId = Guid.Parse(myIdStr);
    ...
    [HttpPost("Private/Initiate")]
    public async Task<IActionResult> InitiatePrivateChat([FromBody] InitiateChatDto dto)
    {
        var myIdStr = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (myIdStr == null) return Unauthorized();
        var myId = Guid.Parse(myIdStr);
        ...
        return Ok(new ChatDto
        {
            ...
            Messages = chat.Messages.OrderBy(m => m.SentAt).Select(m => new MessageDto
            {
                ...
                {
                    Id = m.CustomOffer.Id,
                    Title = m.CustomOffer.Title,
                    Price = m.CustomOffer.Price,
                    Status = m.CustomOffer.Status
                } : null,
                Attachments = m.Attachments.Select(a => new AttachmentDto { Url = a.Url, FileName = a.FileName, FileType = a.FileType, FileSize = a.FileSize }).ToList()
            }).ToList()
        });
...
public class TicketController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] string subject)
    {
        var userIdStr = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (userIdStr == null) return Unauthorized();
